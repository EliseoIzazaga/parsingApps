...
def findTimeSolar(inTimeStampsList):
    #print(inTimeStampsList)
    hours   = 0
    minutes = 0
    seconds = 0
    j = 0
    while j < (len(inTimeStampsList) - 1):
        temp1 = inTimeStampsList[j].split(":")
        tempnext = inTimeStampsList[j + 1].split(":")
        #print(temp1)
        #print(tempnext)
        if(temp1[0] == tempnext[0] ):
            j = j + 1
            #print("First: " + temp1[0])
            #print("Next: " + tempnext[0])
            #print("Result: " + str(int(tempnext[0]) - int(temp1[0])) )
        else:
            if( int(tempnext[0]) - int(temp1[0]) == 1 ):
                #print("Counted the hour: ")
                hours = hours + 1
                #print("Total Hours: " + str(hours))
                j = j + 1
            else:
                j = j  + 1
    #print("To return: " + str(hours))
    temp1 = inTimeStampsList[0].split(":")
    tempLast = inTimeStampsList[len(inTimeStampsList) -1 ].split(":")
    minutes = 60 - int(temp1[1]) + int(tempLast[1])
    if(minutes > 60):
        minutes = minutes - 60
        hours = hours + 1

    print("Total Time HH:MM " + str(hours) + ":" + str(minutes))
    timeToReturn = str(hours) + ":" + str(minutes)
    return timeToReturn
    


    #make counter that checks the first 2 numbers of the list. compares one to the next until ther is a change, when there is a change add it to the hours, 

def findTotalTimeUsingSecs(inTimeStamps):
    print(inTimeStamps)
...
#timeElapsed = findTotalTimeElapsed(timeStamps, VOLT)
timeElapsed = findTimeSolar(timeStamps)
timeElapsed = "Total Time: HH:MM " + str(timeElapsed)
#timeElapsed = "Total Time: HH:MM:SS " + "20:44:46"
#timeElapsed = "TEST NOT COMPLETE, JEITTA OVER TEMP"

#findTotalTimeUsingSecs(timeStamps)
