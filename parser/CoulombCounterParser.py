...
specificLog = "first unit second part"        #log name without file extention (no .txt or .png)
...
with open(logToOpen, encoding="ANSI") as inFile:
    for line in inFile:
        ...
        #print(splitInLine)
        if("bq27z561:" in splitInLine and "read" not in splitInLine):
            #print(read_data)
            #print(splitInLine)
            with open(directoryToSaveNewParsedLog, 'a') as fp:
                neg2inList = splitInLine[-2]
                neg3inList = splitInLine[-3]
                splitInLine[-2] = neg3inList
                splitInLine[-3] = neg2inList

                voltageToAppend = splitInLine[-3] + splitInLine[-2]
                #splitInLine.pop(-1)
                #splitInLine.pop(-2)
                #splitInLine.pop(-3)
                #splitInLine.pop(-4)
                #print(voltageToAppend)
                if(len(voltageToAppend) == 4):  
                    voltageToAppend = int(voltageToAppend, 16)
                #print(str(voltageToAppend))
                #print(str(splitInLine))
                timeStampAndVoltage = []
                
                timeStampAndVoltage.append(splitInLine[1])
                timeStampAndVoltage.append(voltageToAppend)
                #print(str(timeStampAndVoltage))
                #print(neg2inList)
                #print(neg3inList)
                #splitInLine.pop(0)
                #print(timeStampAndVoltage)
                fp.writelines(str(timeStampAndVoltage))
                fp.write("\n")
        if("Estimated" in splitInLine):
            #print(read_data)
            #print(splitInLine)
            with open(directoryToSaveNewParsedLog, 'a') as fp:
                splitInLine.pop(0)
                fp.writelines(splitInLine)
                fp.write("\n")
        #timeStamps = splitInLine[1]
        #print(timeStamps)
